...
class TransactionController : ViewModel() {
    ...
    // Megha Verma
    // https://medium.com/@meghaverma12
    fun syncTransactions(userToken: String, unSyncedTransactions: List<Transaction>) {
        val token = "Bearer $userToken"  // Ensure proper token formatting
        val transactionHolder = TransactionsHolder(transactions = unSyncedTransactions)

        api.syncTransactions(token, transactionHolder).enqueue(object : Callback<SyncResponse> {
                ...
                if (response.isSuccessful) {
                    ...
                    val errorMessage = if (response.errorBody() != null) {
                        try {
                            ...
                            "Error syncing categories: ${errorResponse.message}"
                        } catch (e: Exception) {
